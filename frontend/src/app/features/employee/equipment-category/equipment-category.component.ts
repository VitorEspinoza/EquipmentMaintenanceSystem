...
export class EquipmentCategoryComponent implements OnInit {
  ...
  loadCategories(): void {
    this.employeeService.getEquipmentCategories(!this.isChecked).subscribe({
      next: (response: ApiResponse<EquipmentCategory[]>) => {
  ...
  saveCategory(): void {
    ...
    if (category.id) {
      this.employeeService.updateEquipmentCategory(category).subscribe({
        next: (response: ApiResponse<EquipmentCategory>) => {
          if (response.isSuccess) {
      ...
      this.employeeService.createEquipmentCategory(category).subscribe({
        next: (response: ApiResponse<EquipmentCategory>) => {
          if (response.isSuccess) {
  ...
  deleteCategory(id: number): void {
    this.employeeService.deleteEquipmentCategory(id).subscribe({
      next: () => {
        this.loadCategories();
